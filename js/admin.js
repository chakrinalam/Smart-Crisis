...
async function loadAdminDashboard() {
    ...
    areas.forEach(a => {
        ...
        const barElement = document.querySelector(`.bar[data-area="${a}"]`);
        if (barElement) {
            barElement.setAttribute('data-height', `${finalHeightPct}%`);
            const span = barElement.querySelector('.bar-value');
            if (span) span.textContent = areaCounts[a];
        }
    });
